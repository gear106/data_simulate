...
_UNIPASE_DIR = Path(__file__).resolve().parent.parent / 'unipase-main'
if str(_UNIPASE_DIR) not in sys.path:
    sys.path.insert(0, str(_UNIPASE_DIR))
...
class Model(pl.LightningModule):
    def __init__(self, config):
        ...
        self.semantic_model = WavLM_feat(
            wavlm_ckpt_path=config['dewavlm_ckpt_path'],
            output_layer=[1, 24],
            load_pretrained=True,
            frozen=True,
        )
        self.feat_fusion = nn.Linear(1536, 1536)
    ...
    def extract_semantic_features(self, wavs: torch.Tensor) -> torch.Tensor:
        """extract DeWavLM features and fuse acoustic + phonetic streams"""
        # wavs: (b, t)
        wavs = self.semantic_model.pad(wavs)
        with torch.no_grad():
            # ra: layer 1 (acoustic), rp: layer 24 (phonetic)
            ra, rp = self.semantic_model(wavs, sr=16000, mask=False)
        fused = self.feat_fusion(torch.cat([ra, rp], dim=-1))
        return fused
    ...
    def configure_optimizers(self):
        opt = torch.optim.AdamW(
            list(self.dnn.parameters()) + list(self.feat_fusion.parameters()),
            **self.config['opt']
        )
